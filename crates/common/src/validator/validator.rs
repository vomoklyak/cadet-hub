...
pub fn dot_dd_mm_yyyy_date(date_str: &str) -> Result<(), ValidationError> {
    match date_time_util::dot_dd_mm_yyyy_str_as_utc_timestamp(date_str) {
        Ok(_) => Ok(()),
...
mod test {
    ...
    fn should_validate_full_name_case_invalid_full_name() {
        ...
        assert_that(&result)
            .is_err()
            .is_equal_to(ValidationError::new("error-validation-invalid-full-name"));
    }

    #[test]
    fn should_validate_dot_dd_mm_yyyy_date() {
        // Given
        let date_str = "20.02.2020";

        // When
        let result = dot_dd_mm_yyyy_date(date_str);
    ...
    #[test]
    fn should_validate_consistent_tax_number_with_birth_date_or_passport_number_entry_case_skip_optional_validation(
    ) {
        // Given
        let entry = ImpexCadetCourseEntryBuilder::default()
            .tax_number("invalid")
            .birth_date("20.02.2020")
            .skip_optional_validation(Some(true))
            .build()
            .expect("failed build ImpexCadetCourseEntry");
