...
pub(crate) fn to_utc_timestamp(date: &str) -> Option<i64> {
    ...
    if trimmed_date.is_empty() {
        ...
        date_time_util::dot_dd_mm_yyyy_str_as_utc_timestamp(trimmed_date).ok()
    }
}

pub(crate) fn to_utc_timestamp_next_day(date: &str) -> Option<i64> {
    let trimmed_date = date.trim();
    if trimmed_date.is_empty() {
        None
    } else {
        date_time_util::dot_dd_mm_yyyy_str_as_utc_timestamp_next_day(trimmed_date).ok()
    }
}

pub(crate) fn to_date_str(timestamp: i64) -> Option<String> {
    date_time_util::utc_timestamp_as_dot_dd_mm_yyyy_str(timestamp).ok()
}
