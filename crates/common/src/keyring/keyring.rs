...
pub fn set_admin_key(service: &str, key: &str) -> CadetHubResult<String> {
    set_key(service, ADMIN, key)
}

pub fn set_key(service: &str, user: &str, key: &str) -> CadetHubResult<String> {
    info!("Set key: service={}, user={}", service, user);
...
pub fn delete_admin_key(service: &str) -> CadetHubResult<()> {
    delete_key(service, ADMIN)
}
...
mod tests {
    ...
    fn should_set_key() {
        ...
        let key = set_key(service, user, key).expect("failed get or create key");
    ...
    fn should_delete_key_case_non_existent_key() {
        // Given
        let service = "test-cadet-hub-service-4";
        let user = "test-cadet-hub-admin-4";
        clear_keyring(service, user);
