...
impl DatabaseConfig {
    const SQLITE_SCHEMA: &str = "sqlite:";
    ...
    pub fn url(&self, data_directory_path: PathBuf) -> CadetHubResult<String> {
        match self.url {
            ...
            None => data_directory_path
                .join("data.db")
                .to_str()
                .map(|path_str| format!("{}{}", Self::SQLITE_SCHEMA, path_str))
                .ok_or(
    ...
    pub fn file_path(&self, data_directory_path: PathBuf) -> CadetHubResult<String> {
        self.url(data_directory_path)
            .map(|url| url.replacen(Self::SQLITE_SCHEMA, "", 1))
    }
