...
fn check_checksum(tax_number: &str) -> bool {
    let multipliers: [i32; 9] = [-1, 5, 7, 9, 4, 6, 10, 5, 7];
    let sum: i32 = tax_number
        .chars()
        .take(9)
        .enumerate()
        .map(|(char_index, char)| {
            let char_digit = char.to_digit(10).unwrap() as i32;
            char_digit * multipliers[char_index]
        })
        .sum();
    let checksum = sum.rem_euclid(11).rem_euclid(10) as u32;
    tax_number
        .chars()
        .nth(9)
        .and_then(|char| char.to_digit(10))
        .filter(|digit| digit.eq(&checksum))
        .is_some()
}
...
mod tests {
    ...
    #[test]
    fn should_build_regular_tax_number_type_case_invalid_checksum() {
        // Given
        let tax_number = "4386298728";
        let birth_date = "02.02.2020";
