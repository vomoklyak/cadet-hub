...
impl CadetHubBeError {
    pub fn pretty_debug_str(&self) -> String {
        match self {
            Self::CadetHubError(general_error) => general_error.pretty_debug_str(),
            _ => format!("{:?}", self),
        }
    }
...
impl From<std::io::Error> for CadetHubBeError {
    fn from(error: std::io::Error) -> Self {
        CadetHubError::general_error_with_source(error).into()
    }
}

impl From<zip::result::ZipError> for CadetHubBeError {
