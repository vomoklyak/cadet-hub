...
use crate::util::date_time_util::{
    dot_dd_mm_yyyy_str_as_utc_timestamp, utc_timestamp_as_dot_dd_mm_yyyy_str,
};
...
impl From<&ImpexCadetCourseEntry> for Cadet {
    fn from(value: &ImpexCadetCourseEntry) -> Self {
        ...
        let birth_date = dot_dd_mm_yyyy_str_as_utc_timestamp(value.birth_date())
...
impl From<&ImpexCadetCourseEntry> for CadetCourse {
    fn from(value: &ImpexCadetCourseEntry) -> Self {
        let start_date = dot_dd_mm_yyyy_str_as_utc_timestamp(value.start_date())
            .expect("failed parse start_date");
        let end_date = dot_dd_mm_yyyy_str_as_utc_timestamp(value.end_date())
...
impl From<&CadetCourseEntry> for ImpexCadetCourseEntry {
    fn from(value: &CadetCourseEntry) -> Self {
        let birth_date = utc_timestamp_as_dot_dd_mm_yyyy_str(value.birth_date().clone())
            .expect("failed to convert birth");
        let start_date = utc_timestamp_as_dot_dd_mm_yyyy_str(value.start_date().clone())
            .expect("failed to convert start_date");
        let end_date = utc_timestamp_as_dot_dd_mm_yyyy_str(value.end_date().clone())
